    ...
    return (
        <AppLayout breadcrumbs={[{ title: 'Pagos de Cliente', href: '/customerPayments' }, { title: `Factura Detallada`, href: '#' }]}>            
            ...
            <div className="max-w-3xl mx-auto bg-white shadow rounded-lg p-8 mt-8">
                ...
                {/* Todas las facturas ligadas, incluyendo la principal, una debajo de la otra */}
                <div className="flex flex-col gap-6">
                    {linkedPayments.map((p, idx) => (
                        <div key={p.id} className="border rounded-lg p-4 bg-gray-50">
                            <div className="flex flex-wrap gap-4 mb-2">
                                <div><span className="font-semibold">Acta:</span> {p.acta && p.acta.code ? p.acta.code : (p.id_acta || '-')}</div>
                                <div><span className="font-semibold">Cliente:</span> {p.acta && p.acta.client && p.acta.client.name ? p.acta.client.name : '-'}</div>
                                <div><span className="font-semibold">Técnico:</span> {p.acta && p.acta.creator && p.acta.creator.name ? p.acta.creator.name : '-'}</div>
                                <div><span className="font-semibold">Fecha de Pago:</span> {p.payment_date ? new Date(p.payment_date).toLocaleDateString() : ''}</div>
                                <div><span className="font-semibold">Moneda:</span> {p.currency}</div>
                                <div><span className="font-semibold">Monto:</span> ₡{p.amount?.toLocaleString()}</div>
                                <div><span className="font-semibold">Total Cancelado:</span> {p.is_total ? 'Sí' : 'No'}</div>
                            </div>
                            <div className="flex flex-wrap gap-4 mb-2">
                                ...
                                <div><span className="font-semibold">Documento:</span> {p.document_number}</div>
                                <div><span className="font-semibold">Transacción:</span> {p.transaction_type}</div>
                                <div><span className="font-semibold">Receptor:</span> {p.receiver}</div>
                            </div>
                            <div className="mb-2">
