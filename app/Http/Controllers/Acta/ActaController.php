...
class ActaController extends Controller
{
    ...
    public function createWhitAssigment($id)
    {
        ...
        \Log::info('Creando acta para asignación', [
            'id' => $assignment->id,
            'technician_id' => $assignment->id_technician,
            'customer_id' => $assignment->id_customer,
        ]);

        // Solo el cliente asignado
        $users = User::where('id', $assignment->id_customer)
            ->select('id', 'name', 'contact', 'adress', 'phone', 'code')
            ->get();
    ...
    {
        $acta = Acta::with([
            'components', 'services', 'jobTypes', 'deliveryClasses', 'client', 'technician'
        ])->findOrFail($id);
        ...
        $actaData = [
            ...
            'services' => $acta->services->pluck('id')->toArray(),
    ...
    public function update(Request $request, $id)
    {
        ...
        $acta->fill($validated);
        $acta->save();
        $acta->jobTypes()->sync($validated['job_type'] ?? []);
        $acta->deliveryClasses()->sync($validated['delivery_class'] ?? []);
        $acta->services()->sync($validated['services'] ?? []);
